...
QueueArray <byte> queue(14);
QueueArray <byte> pathQueue(14);
...
void appendDestination(byte xdes,byte ydes, boolean middleSquare){
    ...
    flood[7][6]=0;
    flood[6][6]=0;
//add element use push()
// push an item to the queue.  -   bool push (const T i);
    ////queue.enqueue(7);
    queue.push(7);
    queue.push(7);
    queue.push(6);
...
// pop an item from the queue.   -   T pop ();

     while (!queue.isEmpty ()){ //remove item use pop()
        //byte yrun=queue.dequeue();
        //byte xrun=queue.dequeue();
        byte yrun=queue.pop();
        byte xrun=queue.pop();
